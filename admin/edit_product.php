...
<title>تعديل المنتج | توب سودان</title>
...
/*========================================================
  TOOB SUDAN — EDIT PRODUCT · PREMIUM ADMIN UI
========================================================*/
...
.section-card:nth-of-type(5){animation-delay:.33s;}
...
/* price fields with currency indicator */

.price-shell{position:relative;}

.price-shell input{padding-left:52px;}
...
.discount-hint.show{display:flex;}
...
#preview.fade-pop{animation:popFade .5s var(--ease);}

@keyframes popFade{

  0%{opacity:.2;transform:scale(.94);}
...
.upload-zone p{color:var(--muted);font-weight:600;font-size:12.5px;}
...
.btn-save.is-loading{pointer-events:none;color:transparent;}
...
.danger-card{
  ...
  border:1.6px solid rgba(200,50,50,.28);
  border-radius:var(--radius-lg);
  padding:28px 32px;
  margin-top:24px;
  display:flex;
...
.btn-delete{
  ...
  gap:9px;
  transition:.35s var(--ease);
  box-shadow:0 10px 22px rgba(200,50,50,.25);
  white-space:nowrap;
...
.btn-delete:hover{
  ...
  transform:translateY(-3px);
  box-shadow:0 14px 28px rgba(200,50,50,.35);
...
@media(max-width:640px){
  ...
  .danger-card{flex-direction:column;align-items:stretch;text-align:center;}
...
<div class="page-wrap">
  ...
  <div class="top-bar">
    <div class="top-bar-row">
      <div class="top-bar-titles">
        <h1><i class="fa-solid fa-pen"></i> تعديل المنتج</h1>
        <p class="sub">قم بتعديل بيانات المنتج وحفظ التغييرات</p>
  ...
  <form method="POST" enctype="multipart/form-data" id="editProductForm" novalidate>
      ...
      <div class="grid">
        ...
        <div class="field">
          ...
          <div class="price-shell">
            <span class="cur-tag">ج.س</span>
            <input type="number" step="0.01" name="price" id="priceInput" value="<?php echo $row['price']; ?>" required>
          </div>
        ...
        <div class="field">
          ...
          <div class="price-shell">
            <span class="cur-tag">ج.س</span>
            <input type="number" step="0.01" name="old_price" id="oldPriceInput" value="<?php echo $row['old_price']; ?>">
          </div>
          <span class="discount-hint" id="discountHint"><i class="fa-solid fa-tag"></i> <span id="discountVal"></span></span>
        </div>
    ...
    <div class="section-card">
      <div class="section-head">
        ...
        <div>
          <h2>تفاصيل المنتج والمخزون</h2>
          <p>المواصفات الدقيقة والكمية المتاحة</p>
        </div>
      </div>
      ...
      <div class="grid grid-3">
        ...
        <div class="field">
          ...
          <select name="size">
            ...
            <option value="Free Size" <?php if($row['size']=="Free Size") echo "selected"; ?>>Free Size</option>
          </select>
        </div>
        ...
        <div class="field">
          ...
          <select name="gender">
            <option value="نساء" <?php if($row['gender']=="نساء") echo "selected"; ?>>نساء</option>
            <option value="رجال" <?php if($row['gender']=="رجال") echo "selected"; ?>>رجال</option>
            <option value="أطفال" <?php if($row['gender']=="أطفال") echo "selected"; ?>>أطفال</option>
            <option value="للجميع" <?php if($row['gender']=="للجميع") echo "selected"; ?>>للجميع</option>
          </select>
        </div>
        ...
        <div class="field">
          ...
          <input type="text" name="occasion" value="<?php echo $row['occasion']; ?>">
        </div>

        <div class="field">
          <label><i class="fa-solid fa-soap"></i> تعليمات الغسيل</label>
          <input type="text" name="care" value="<?php echo $row['care']; ?>">
        </div>
    ...
    <div class="section-card">
      ...
      <div class="field full">
        <textarea name="description" placeholder="اكتب وصفاً جذاباً للمنتج..."><?php echo $row['description']; ?></textarea>
      </div>
    </div>
    ...
    <div class="section-card">
      <div class="section-head">
        ...
        <div>
          <h2>صورة المنتج</h2>
          <p>الصورة الحالية — اختر صورة جديدة لاستبدالها</p>
        </div>
      </div>
      ...
      <div class="field full">
        <label class="upload-zone" id="uploadZone">
          ...
          <span class="current-tag" id="currentTag"><i class="fa-solid fa-check"></i> الصورة الحالية</span>
          ...
          <h3>اضغط أو اسحب لاختيار صورة جديدة</h3>
          <p>JPG - PNG - WEBP</p>
        ...
        <div class="checks">
          ...
          <label class="check-pill">
            <input
              type="checkbox"
              name="is_featured"
              <?php if($row['is_featured']) echo "checked"; ?>>
            <span>⭐ منتج مميز</span>
          </label>
          ...
          <label class="check-pill">
            <input
              type="checkbox"
              name="is_new"
              <?php if($row['is_new']) echo "checked"; ?>>
            <span>🆕 منتج جديد</span>
          </label>
    ...
    <div class="action-bar">
      ...
      <button class="btn btn-save" name="update" id="saveBtn" type="submit">
        <span class="spinner"></span>
        <span class="btn-label"><i class="fa-solid fa-floppy-disk"></i> حفظ التعديلات</span>
      </button>
  ...
  <div class="danger-card">
    <div class="danger-text">
      <h3><i class="fa-solid fa-triangle-exclamation"></i> منطقة خطرة</h3>
      <p>حذف المنتج إجراء نهائي ولا يمكن التراجع عنه.</p>
    </div>
    <a
...
function previewImage(event){
  ...
  reader.onload = function(){
    ...
    previewImg.classList.remove('fade-pop');
    void previewImg.offsetWidth;
    previewImg.classList.add('fade-pop');
    currentTag.innerHTML = '<i class="fa-solid fa-image"></i> صورة جديدة (لم تُحفظ بعد)';
  };
...
function updateDiscountHint(){
  ...
  if(oldPrice > price && price > 0){
    ...
    document.getElementById('discountVal').textContent = 'خصم ' + pct + '% عن السعر السابق';
...
document.getElementById('oldPriceInput').addEventListener('input', updateDiscountHint);
updateDiscountHint();
...
document.querySelectorAll('.btn, .btn-delete').forEach(btn=>{
